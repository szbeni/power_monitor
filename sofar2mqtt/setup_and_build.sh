...
Standalone Sofar2mqtt for NodeMCU. Default port: /dev/ttyUSB0
Serial monitor is 9600 baud (matches upstream sketch).
EOF
}
...
echo "==> Building sofar2mqtt (nodemcuv2)"
pio run
...
if [[ "$MONITOR" -eq 1 ]]; then
  pio device monitor --port "$UPLOAD_PORT" -b 9600
fi
